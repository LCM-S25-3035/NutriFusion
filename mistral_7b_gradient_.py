# -*- coding: utf-8 -*-
"""Mistral_7b gradient .ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JWkRU8AicUH6lCYUBml-RUHLpG5bxWfa
"""
...
generator = pipeline("text-generation", model="tiiuae/falcon-rw-1b", device_map="auto")
output = generator("You are a helpful nutritionist. Recommend diet for butter chicken and diabetes:", max_new_tokens=100)
...
# Authenticate
login("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")
...
tokenizer = AutoTokenizer.from_pretrained(model_name, token="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")
mistral_model = AutoModelForCausalLM.from_pretrained(
    model_name,
    quantization_config=bnb_config,
    device_map="auto",
    token="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"
)
...
!pip install transformers==4.35.2 sentence-transformers faiss-cpu accelerate bitsandbytes --quiet

from huggingface_hub import login
from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig
import torch

# Login (don't share this token publicly)
login("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")
...
def generate_recipe_review(dish_name, age, disease, cleaned_ingredients, nutrient_goals):
    allowed = ['turmeric', 'olive oil', 'cinnamon', 'spinach', 'broccoli', 'kale', 'bell peppers',
               'mushrooms', 'zucchini', 'flaxseeds', 'chia seeds', 'whole grain pasta']
...
⚠️ STRICT RULES:
- You may only REMOVE ingredients from this list above.
- You may only ADD ingredients from this list: {', '.join(allowed)}.
- Do NOT mention any ingredient not in these lists.
- Do NOT invent ingredients or swaps that don't follow the rule above.
...
"""
    ...
    outputs = mistral_model.generate(**inputs, max_new_tokens=350)
    return tokenizer.decode(outputs[0], skip_special_tokens=True)

!pip install gradio --quiet
import gradio as gr

def ui_infer(dish_name, age, disease):
    recipe = recipe_df[recipe_df['RecipeName'].str.lower().str.contains(dish_name.lower())].iloc[0]
    ingredients = recipe['Cleaned_Ingredients']

    user_match = health_df[(health_df['Ages'] == int(age)) & (health_df['Disease'].str.contains(disease, case=False))]
    if user_match.empty:
        return "⚠️ No matching user found."
    nutrient_goals = user_match.iloc[0][['Calories', 'Protein', 'Carbohydrates', 'Fat', 'Fiber']].to_dict()

    return generate_recipe_review(dish_name, age, disease, ingredients, nutrient_goals)

gr.Interface(fn=ui_infer,
             inputs=["text", "number", "text"],
             outputs="text",
             title="NutriFusion - Personalized Recipe Review",
             description="Get a health-aware recipe review for a dish based on your age and health condition"
).launch(share=True)

def ui_infer(dish_name, age, disease):
    try:
        # Find recipe by name
        recipe_match = recipe_df[recipe_df['RecipeName'].str.lower().str.contains(dish_name.lower())]
        if recipe_match.empty:
            return f"⚠️ No recipe found for: {dish_name}"
        ingredients = recipe_match.iloc[0]['Cleaned_Ingredients']

        # Match health goal
        user_match = health_df[(health_df['Ages'] == int(age)) &
                               (health_df['Disease'].str.lower() == disease.lower())]
        if user_match.empty:
            return f"⚠️ No nutrient goal found for Age {age} and Condition '{disease}'"
...
recipe_df = pd.read_csv("/content/70000_recipes_nutrients.csv")
health_df = pd.read_csv("/content/health_age_data_70000_synthetic.csv")

# Optional: See sample values to help debugging
print(recipe_df[['RecipeName', 'Cleaned_Ingredients']].head())
print(health_df[['Ages', 'Disease']].drop_duplicates().head())
...
def ui_infer(dish_name, age, disease):
    # Match dish name from recipe dataset
    matched_recipe = recipe_df[recipe_df['recipe_name'].str.lower().str.contains(dish_name.lower())]
    if matched_recipe.empty:
        return f"⚠️ No matching recipe found for '{dish_name}'."

    recipe = matched_recipe.iloc[0]
    ingredients = recipe['ingredients']

    # Match user from health dataset
    user_match = health_df[(health_df['Ages'] == int(age)) & (health_df['Disease'].str.contains(disease, case=False))]
    if user_match.empty:
        return f"⚠️ No health record found for age {age} and disease '{disease}'."
...
def generate_recipe_review(dish_name, age, disease, ingredients, nutrient_goals):
    allowed_add = {"turmeric", "olive oil", "cinnamon", "spinach", "broccoli", "kale", "bell peppers", "mushrooms", "zucchini", "flaxseeds", "chia seeds", "whole grain pasta"}

    # Split ingredients and clean them
    ingredient_list = [i.strip().lower() for i in ingredients.split(",")]

    # Remove restricted ingredients (based on simple disease logic — here we remove pork, pancetta, bacon, wine, cheese for diabetes)
    removed = []
    for item in ingredient_list:
        if any(forbidden in item for forbidden in ['bacon', 'pork', 'pancetta', 'wine', 'cheese']):
            removed.append(item)

    # New list without removed items
    modified_ingredients = [i for i in ingredient_list if i not in removed]

    # Add diabetes-safe ingredients
    additions = sorted(list(allowed_add - set(modified_ingredients)))
    modified_ingredients.extend(additions)

    # Final cleaned list
    cleaned_list = ", ".join(modified_ingredients)

    # Generate explanation
    explanation = (
        f"The original recipe for {dish_name} includes ingredients such as {ingredients}. "
        f"To make this dish more suitable for a {age}-year-old with {disease}, we removed ingredients like {', '.join(removed)} "
        f"as they can negatively impact blood sugar or heart health. Instead, we have added diabetes-friendly ingredients like "
        f"{', '.join(additions)}, which are rich in fiber, antioxidants, and essential nutrients that help regulate blood sugar and support overall health. "
        f"These changes result in a more balanced and nourishing dish that remains delicious and appropriate for someone managing {disease}."
    )

    return cleaned_list, explanation
...
def ui_infer(dish_name, age, disease):
    ...
    user_match = health_df[(health_df['Ages'] == int(age)) & (health_df['Disease'].str.contains(disease, case=False))]
    if user_match.empty:
        return f"⚠️ No health data found for age {age} and disease '{disease}'.", ""
...
gr.Interface(
    ...
    inputs=[
        ...
        gr.Number(label="Age"),
        gr.Textbox(label="Health Condition")
    ],
    outputs=[
        gr.Textbox(label="✅ Modified Ingredient List"),
        gr.Textbox(label="📋 Nutritionist Explanation")
    ],
    title="NutriFusion - Personalized Recipe Review",
    description="Enter a dish name, age, and health condition to receive a safe and professional recipe review"
).launch(share=True)